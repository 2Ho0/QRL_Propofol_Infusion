...
A hybrid Quantum-Classical Reinforcement Learning system for closed-loop BIS-guided propofol anesthesia control, featuring VitalDB real patient data integration and 2-stage training (Offline BC/CQL → Online RL).
...
- **VitalDB Integration**: Train on 6,388 real surgical patients' data
...
Train on expert demonstrations from VitalDB dataset:
...
# BC + CQL offline training (more conservative)
python experiments/train_hybrid.py \
    --n_cases 100 --offline_epochs 50 --online_episodes 500 \
    --use_cql --cql_alpha 1.0

# With LSTM temporal encoder
python experiments/train_hybrid.py \
    --n_cases 100 --offline_epochs 50 --online_episodes 500 \
    --encoder lstm
```
...
python experiments/compare_quantum_vs_classical_dualdrug.py \
    ...
    --cql_temp 1.0 --cql_num_random 10
```

**CQL Parameters**:
- `--use_cql`: Enable Conservative Q-Learning
- `--bc_weight`: Weight for BC loss (0.0-1.0, default: 1.0)
- `--cql_alpha`: CQL penalty weight (default: 1.0)
- `--cql_temp`: Temperature for logsumexp (default: 1.0)
- `--cql_num_random`: Number of random actions for penalty (default: 10)

#### 3. **Pure Online Training** (Simulator Only)

Train from scratch without VitalDB data:
...
[VitalDB](https://vitaldb.net/) is an open clinical dataset with 6,388 surgical patients' intraoperative data:
- **BIS monitoring** with signal quality indices
- **TCI pump data**: Propofol/Remifentanil effect-site concentrations, infusion rates
- **Vital signs**: HR, BP, SpO2
- **Demographics**: Age, sex, height, weight
- **Expert drug dosing histories** from anesthesiologists

### Data Statistics (Dual Drug Cases)

Successfully loaded **2,830 transitions from 5 VitalDB cases**:

**Propofol Rates**:
- Range: 0.0 - 27.9 mg/kg/h
- Mean: 6.1 ± 5.2 mg/kg/h

**Remifentanil Rates**:
- Range: 0.02 - 2.75 μg/kg/min  
- Mean: 0.63 ± 0.59 μg/kg/min

**BIS Values**:
- Range: 35 - 96
- Mean: 49 ± 12 (well-controlled anesthesia depth)

**Computed Rewards**:
- Range: -1.740 to 0.995
- Mean: -0.083 ± 0.329
- Real reward calculation with BIS tracking, drug efficiency, stability, and safety components
